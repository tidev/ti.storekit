...
- (NSDictionary *)dictionaryFromPlistData:(NSData *)data
{
    ...
    if (!dictionaryParsed) {
        if (error) {
            NSLog(@"[ERROR] Error creating dictionary from Plist: %@", error.description);
        }
    }
...
{
    if (![self isTransactionAndItsReceiptValid])
    {
        // There was something wrong with the transaction we got back, so no need to call verifyReceipt.
        if (error) {
            *error = [NSError errorWithDomain:SKErrorDomain code:SKErrorPaymentInvalid userInfo:nil];
        }
        return NO;
    }
    ...
    NSString *payload = (sharedSecret == nil)
    ? [NSString stringWithFormat:@"{\"receipt-data\":\"%@\"}",jsonObjectString]
    : [NSString stringWithFormat:@"{\"receipt-data\":\"%@\",\"password\":\"%@\"}",jsonObjectString, sharedSecret];
    
    NSData *payloadData = [payload dataUsingEncoding:NSUTF8StringEncoding];
    
    NSString *serverURL = sandbox ? ITMS_SANDBOX_VERIFY_RECEIPT_URL : ITMS_PROD_VERIFY_RECEIPT_URL;
    
    NSLog(@"[DEBUG] Verifying receipt with %@", serverURL);
...
- (BOOL)isTransactionAndItsReceiptValid
{
    ...
    if (!receiptDict) {
        NSLog(@"[ERROR] No receipt dictionary found");
        return NO;
    }
    ...
    if (!purchaseInfoDict) {
        NSLog(@"[ERROR] No purchase information dictionary found");
        return NO;
    }
    ...
    if (![self isTransactionIdUnique:transactionId])
    {
        ...
        NSLog(@"[ERROR] Transaction identifer is not unique");
        return NO;
    }
    ...
    if (!result)
    {
        NSLog(@"[ERROR] Failed receipt security check");
        return NO;
    }
    ...
    {
        NSLog(@"[ERROR] Transaction details do not match purchase information");
        return NO;
    }
...

{
    ...
    if (productIdentifier && ![productIdentifier isEqualToString:[purchaseInfoDict objectForKey:@"product-id"]])
    {
        failCount++;
    }
    
    if (quantity && quantity != [[purchaseInfoDict objectForKey:@"quantity"] intValue])
    {
        failCount++;
    }
    
    if (transactionIdentifier && ![transactionIdentifier isEqualToString:[purchaseInfoDict objectForKey:@"transaction-id"]])
    {
        failCount++;
    }
...
- (BOOL)doesTransactionInfoMatchReceipt:(NSString*) receiptString
{
    ...
    NSDictionary *verifiedReceiptDictionary = [self dictionaryFromJSONData:receiptString];
    if (!verifiedReceiptDictionary) {
        NSLog(@"[ERROR] Verified receipt dictionary is empty");
        return NO;
    }
    ...
    {
        NSLog(@"[ERROR] Invalid status code: %d", verifyReceiptStatus);
        if (verifyReceiptStatus == 21007) {
            NSLog(@"[ERROR] The receipt is a sandbox receipt, but is was sent to the production service for verification");
            NSLog(@"[ERROR] Set the 'receiptVerificationSandbox' property to true");
        } else if (verifyReceiptStatus == 21008) {
            NSLog(@"[ERROR] The receipt is a production receipt, but is was sent to the sandbox service for verification");
            NSLog(@"[ERROR] Set the 'receiptVerificationSandbox' property to false");
        }
        return NO;
    }
    ...
    if (!purchaseInfoFromTransaction)
    {
        ...
        NSLog(@"[ERROR] No receipt found for transaction");
        return NO;
    }
    ...
    {
        NSLog(@"[ERROR] Receipt 'bid' does not match");
        failCount++;
    }
    ...
    {
        NSLog(@"[ERROR] Receipt 'product_id' does not match");
        failCount++;
    }
    ...
    {
        NSLog(@"[ERROR] Receipt 'quantity' does not match");
        failCount++;
    }
    ...
    {
        NSLog(@"[ERROR] Receipt 'item_id' does not match");
        failCount++;
    }
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    NSError *error = isOk ? nil : [NSError errorWithDomain:SKErrorDomain code:SKErrorPaymentInvalid userInfo:nil];
...
outLabel:
    ...
    if (purchase_info_bytes) free(purchase_info_bytes);
    if (signature_bytes) free(signature_bytes);
    if (receipt_signing_key) CFRelease(receipt_signing_key);
...
- (NSString *)encodeBase64:(const uint8_t *)input length:(NSInteger)length
{
    ...
    size_t outsize = TiStoEstimateBas64EncodedDataSize((size_t)length);
...
- (NSString *)decodeBase64:(NSString *)input
{
    ...
    size_t outsize = TiStoEstimateBas64DecodedDataSize(length);
    char *base64Result = malloc(sizeof(char)*outsize);
...
void * base64_decode(const char* s, size_t * data_len)
{
    ...
    size_t outsize = TiStoEstimateBas64DecodedDataSize(length);
    char *base64Result = malloc(sizeof(char)*outsize);
    bool result = TiStoBase64DecodeData(s, length, base64Result, &outsize);
    if (result) {
        if (data_len) {
            *data_len = outsize;
        }
